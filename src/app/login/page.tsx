...
export default function LoginPage() {
  ...
  const supabaseResetPassword = supabaseAuth?.resetPassword
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    setInfo('')
    if (!supabaseConfigured || !supabaseResetPassword) {
      setError('Password reset requires Supabase auth. Use the Supabase dashboard, or contact support.')
      return
    }
    setLoading(true)
    try {
      const { error: err } = await supabaseResetPassword(email)
      if (err) throw new Error(err)
      setInfo(
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-gradient-to-br from-slate-900 via-slate-800/95 to-slate-900 flex items-center justify-center pt-16 px-4 pb-[max(2rem,env(safe-area-inset-bottom))] md:pb-16">
      <div className="max-w-md w-full p-4 sm:p-6 md:p-8 bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/50 text-slate-100">
        <h1 className="text-2xl sm:text-3xl font-bold mb-2 bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
        ...
        {mode === 'login' ? (
          ...
          <form onSubmit={handleForgotSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full min-h-[44px] bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white px-4 py-3 rounded-xl disabled:opacity-50 font-medium"
            >
