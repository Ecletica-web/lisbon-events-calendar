...
function ReviewEventCard({
  ...
  return (
    <article className="rounded-2xl bg-slate-800 border border-slate-700 overflow-hidden shadow-lg">
      <div className="aspect-[4/3] relative bg-slate-800 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3">
          <h2 className="font-bold text-white text-lg line-clamp-2 drop-shadow-lg">{item.title}</h2>
          <p className="text-slate-200/90 text-sm truncate">{item.venueName || '—'}</p>
          {startDate && (
            <p className="text-slate-300/80 text-xs mt-0.5">
              {startDate.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
            </p>
          )}
        </div>
        {item.validationStatus && (
          <span className="absolute top-2 right-2 px-2 py-1 rounded text-xs bg-amber-600/90 text-white">
            {item.validationStatus}
          </span>
        )}
      </div>
      <div className="p-4 space-y-3">
        {item.descriptionLong && (
          <p className="text-slate-300 text-sm leading-relaxed line-clamp-4">{item.descriptionLong}</p>
        )}
        {(item.tags?.length ?? 0) > 0 && (
          <div className="flex flex-wrap gap-1">
            {item.tags.slice(0, 4).map((tag) => (
              <span key={tag} className="px-2 py-0.5 rounded text-xs bg-slate-700 text-slate-200">{tag}</span>
            ))}
          </div>
        )}
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-8 text-center text-slate-400">
          ...
          <p className="text-sm mt-1">Set EVENT_REVIEW_*_CSV_URL env vars or upload a CSV above.</p>
        </div>
      ) : (
