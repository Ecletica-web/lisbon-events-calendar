...
export async function GET(request: NextRequest) {
  ...
  const { data: memberships } = await db.from('chat_members').select('chat_id').eq('user_id', user.id)
  ...
  const withMembers = await Promise.all(
    (chats ?? []).map(async (chat) => {
      const { data: members } = await db.from('chat_members').select('user_id').eq('chat_id', chat.id)
      const userIds = (members ?? []).map((m) => m.user_id)
      const { data: profiles } = await db.from('user_profiles').select('id, display_name, username, avatar_url').in('id', userIds)
...
export async function POST(request: NextRequest) {
  ...
  const { data: chat, error: chatError } = await db.from('chats').insert({ name: name || null, is_group: isGroup }).select('id, name, is_group, created_at').single()
  ...
  const { error: membersError } = await db.from('chat_members').insert(uniq.map((user_id) => ({ chat_id: chat.id, user_id })))
